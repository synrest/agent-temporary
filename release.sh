...
cp bootstrap/install.sh "$out/bootstrap/install.sh"
printf '%s\n' "$version" > "$out/VERSION"
...
EOF
chmod 755 "$out/install.sh" "$out/uninstall.sh" "$out/agent-temporary" "$out/bootstrap/install.sh"
(cd "$out" && shasum -a 256 agent-temporary agent-temporary-expire.service agent-temporary-expire.timer agent-temporary-boot.service agent-temporary-boot.openrc agent-temporary-expire.openrc macos/agent-temporary-macos macos/agent-temporary-reaper macos/com.agent-temporary.expire.plist macos/com.agent-temporary.boot.plist bootstrap/install.sh VERSION install.sh uninstall.sh > SHA256SUMS)
