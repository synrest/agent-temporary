...
assert_ttl() {
    ...
    activated=$(sed -n 's/^issued_at=//p' "$STATE_DIR/state")
...
assert_absent_field() { ! grep -q "^$1=" "$STATE_DIR/state"; }
assert_ttl 5m 300
assert_ttl 50m 3000
assert_ttl 1h 3600
assert_ttl 8h 28800
grep -qx 'persist_reboot=no' "$STATE_DIR/state"
grep -q '^issued_at=' "$STATE_DIR/state"
assert_absent_field activated_at
grep -qx 'boot_identity=1788047880:142950' "$STATE_DIR/state"
! grep -q '^boot_identity=.*=' "$STATE_DIR/state"
...
if run on --ttl 5m --persist-reboot --persist-reboot >/dev/null 2>&1; then exit 1; fi
assert_absent
run on --ttl 50m --persist-reboot >/dev/null
persistent_issued=$(sed -n 's/^issued_at=//p' "$STATE_DIR/state")
persistent_expiry=$(sed -n 's/^expires_at=//p' "$STATE_DIR/state")
grep -qx 'persist_reboot=yes' "$STATE_DIR/state"
run on --ttl 8h --persist-reboot >"$TMP/persistent-repeat"; grep -q 'already active' "$TMP/persistent-repeat"
[ "$(sed -n 's/^issued_at=//p' "$STATE_DIR/state")" = "$persistent_issued" ]
[ "$(sed -n 's/^expires_at=//p' "$STATE_DIR/state")" = "$persistent_expiry" ]
if run on --ttl 8h >/dev/null 2>&1; then exit 1; fi
[ "$(sed -n 's/^persist_reboot=//p' "$STATE_DIR/state")" = yes ]
run --boot-revoke >/dev/null
grep -qx 'persist_reboot=yes' "$STATE_DIR/state"
[ "$(sed -n 's/^issued_at=//p' "$STATE_DIR/state")" = "$persistent_issued" ]
[ "$(sed -n 's/^expires_at=//p' "$STATE_DIR/state")" = "$persistent_expiry" ]
rm -f "$RULE"; run --boot-revoke >/dev/null
grep -qx 'alice ALL=(ALL) NOPASSWD: ALL' "$RULE"
[ "$(sed -n 's/^expires_at=//p' "$STATE_DIR/state")" = "$persistent_expiry" ]
set_state_field expires_at 1; run --boot-revoke >/dev/null 2>&1 || true; assert_absent
run on --ttl 50m --persist-reboot >/dev/null
set_state_field issued_at 9999999999; run --boot-revoke >/dev/null 2>&1 || true; assert_absent
run on --ttl 50m --persist-reboot >/dev/null; rm -f "$RULE"
if FAKE_LAUNCH_MODE=not-running run --boot-revoke >/dev/null 2>&1; then exit 1; fi
assert_absent
FAKE_LAUNCH_MODE=ok
run on --ttl 50m --persist-reboot >/dev/null; run off >/dev/null; run --boot-revoke >/dev/null; assert_absent
run on --ttl 5m >/dev/null; run --boot-revoke >/dev/null; assert_absent
printf '%s\n' 'phase=active' 'user=alice' 'issued_at=1' 'expires_at=2' 'boot_identity=boot-test' 'persist_reboot=maybe' >"$STATE_DIR/state"
printf '%s\n' 'Defaults:alice !authenticate' 'alice ALL=(ALL) NOPASSWD: ALL' >"$RULE"
run --boot-revoke >/dev/null 2>&1 || true; assert_absent
printf '%s\n' 'Defaults:alice !authenticate' 'alice ALL=(ALL) NOPASSWD: ALL' >"$RULE"
rm -f "$STATE_DIR/state"; run --boot-revoke >/dev/null; [ ! -e "$RULE" ]
