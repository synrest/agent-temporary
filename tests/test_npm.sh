...
ARCHIVE=$ROOT/dist/agent-temporary-0.7.0.tgz
...
"$NODE" -e 'const p=require(process.argv[1]); const keys=Object.keys(p.bin||{}); if(keys.length!==1 || keys[0]!=="agent-temporary-setup" || p.bin["agent-temporary"] || p.version!=="0.7.0") process.exit(1)' "$PKG/package.json"
[ "$(cat "$PKG/payload/VERSION")" = 0.7.0 ]
...
grep -qx 'agent-temporary-setup 0.7.0' "$TMP/version"
...
run_setup status >"$TMP/status"
grep -q 'installed system version=0.7.0' "$TMP/status"
