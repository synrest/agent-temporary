...
mkdir -p "$BIN" "$WORK"
cp "$ROOT/dist/agent-temporary-0.7.2.zip" "$TMP/archive.zip"
...
case "$url" in *latest) printf '%s' 'http://example/releases/tag/v0.7.2';; *zip.sha256) cp "$FAKE_ARCHIVE.sha256" "$out";; *zip) cp "$FAKE_ARCHIVE" "$out";; esac
EOF
chmod 755 "$BIN/curl"
...
: >"$LOG"; printf '%064d  agent-temporary-0.7.2.zip\n' 0 >"$TMP/archive.zip.sha256"
